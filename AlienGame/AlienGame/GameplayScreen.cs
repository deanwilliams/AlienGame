...
namespace AlienGame
{
    class GameplayScreen : GameScreen
    {
        ...
        /// <summary>
        /// Returns an instance of usable player bullet.
        /// Prefers re-using an existing (dead) bullet over creating a new instance
        /// </summary>
        /// <returns>A bullet ready to place onto the world</returns>
        ...
        void Respawn(float elapsed)
        {
            ...
            if (!player.IsAlive)
            {
                ...
                if (player.RespawnTimer <= 0.0f)
                {
                    ...
                    {
                        if (alienBullets[i].IsAlive == false) continue;
                    ...
                    player.Velocity = Vector2.Zero; player.Lives--;
                }
            }
        }
        ...
        void UpdateBullets(float elapsed)
        {
            for (int i = 0; i < playerBullets.Count; ++i)
            {
                ...
                {
                    playerBullets[i].IsAlive = false; hitStreak = 0;
                }
            }
            for (int i = 0; i < alienBullets.Count; ++i)
            {
        ...
        private void UpdateAliens(float elapsed)
        {
            ...
            for (int i = 0; i < aliens.Count; ++i)
            {
                ...
                if ((aliens[i].Position.X < -aliens[i].Width - 64 && aliens[i].Velocity.X < 0.0f) || (aliens[i].Position.X > worldBounds.Width + 64 && aliens[i].Velocity.X > 0.0f))
                {
                ...
                if (aliens[i].FireTimer <= 0.0f && aliens[i].FireCount > 0)
                {
                    if (player.IsAlive)
                    {
                        ...
                        bullet.Position.X = aliens[i].Position.X + aliens[i].Width / 2 - laserTexture.Width / 2; bullet.Position.Y = aliens[i].Position.Y + aliens[i].Height;
        ...
        { 
            for (int i = 0; i < playerBullets.Count; ++i) 
            { 
                if (playerBullets[i].IsAlive) ScreenManager.SpriteBatch.Draw(bulletTexture, playerBullets[i].Position, Color.White);
            } 
            for (int i = 0; i < alienBullets.Count; ++i) 
            { 
        ...
        void DrawHud()
        {
            ...
            else
            {
                ...
                Vector2 size = scoreFont.MeasureString(text); size *= scale;
                // Lives 
